                                    ...
                                    <el-card shadow="never">
                                        <div v-if="activeTabName=='fixedFlowrate'">
                                            ...
                                            <p class="bold fs16 mt8">
                                                ...
                                                <span class="normal break">{{ getMoney(info.maxAmount - senderBalance) }}</span> {{ unit }}
                                            </p>
                                            <p class="bold fs16 mt8">
...
export default {
    ...
    methods: {
        ...
        withdraw() {//提取金额
            ...
            if (state == 0){//若状态为0，提现时输入金额，发起投票
                this.$prompt(this.$t('streamInfo.function.withdraw'), this.$t('streamInfo.function.operation'), {
                    ...
                    let res = await this.allInstance.sablierInstance.methods.withdrawFromStream(this.streamId, setMoneyWeb3(value))
            ...
            } else if (state == 1){//若状态为1，只能查看投票状态
                this.$alert('提现金额：500<br>距离投票结束还剩：10小时', this.$t('streamInfo.function.operation'), {
                    confirmButtonText: '稍后再看',
                    type: 'warning',
                    dangerouslyUseHTMLString: true,
                })
            } else if (state == 2) {//若状态为2，可以提现
                this.$confirm('可提现金额：500，确认是否要提现？', this.$t('streamInfo.function.operation'), {
                    confirmButtonText: this.$t('streamInfo.function.confirm'),
                    cancelButtonText: this.$t('streamInfo.function.cancel'),
                    dangerouslyUseHTMLString: true,
                    type: 'success'
                }).then(async () => {
                    let res = await this.allInstance.sablierInstance.methods.cancelStream(this.streamId)
                        .send({gas: 500000, from: this.user})
                    if (res) {
                        clearInterval(timer)
        ...
        addAmount() {//增加最大金额
            this.$prompt('请输入投资金额：', this.$t('streamInfo.function.operation'), {
                confirmButtonText: this.$t('streamInfo.function.confirm'),
                cancelButtonText: this.$t('streamInfo.function.cancel'),
                inputPattern: /^([0-9]*|[0-9]*.[0-9]+)$/,
                inputErrorMessage: this.$t('streamInfo.function.format'),
